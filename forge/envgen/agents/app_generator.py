...
_BACKEND_SYSTEM = (
    ...
    "  - /forge/reset MUST first set `_FORGE_CLOCK = 0` and `_ID_COUNTERS.clear()`,\n"
    "    then re-seed, so a reset re-initialize()s the universe to a byte-identical\n"
    "    initial state (same rows, same ids, same counters) every time.\n"
    "\n"
