...
_PLAN_SYSTEM = (
    ...
    "  requirements.txt — list: fastapi, uvicorn[standard], sqlalchemy, redis,\n"
    "                     httpx, python-multipart, pydantic. The Forge build pipeline\n"
    "                     replaces it with a pinned, hashed lock of exactly these,\n"
    "                     so the app may import nothing else outside the stdlib.\n"
...
_BACKEND_SYSTEM = (
    ...
    "  - DEPENDENCIES ARE FIXED: import only the standard library plus fastapi, starlette,\n"
    "    pydantic, sqlalchemy, uvicorn, and redis. The container installs a pinned lock of\n"
    "    exactly these, so any other package fails the build.\n"
    ...
    "      POST /forge/reset          → optional body {\"seed\": <int>}. With a seed,\n"
    "                                   delegate to STATE.seed_state(seed); with no body/seed,\n"
    "                                   delegate to STATE.reset_state(). Return {\"ok\": true}.\n"
    "                                   Accept a missing/empty body (seed defaults to None).\n"
    ...
    "          # restore the fixed baseline universe: call self.seed_state(<DEFAULT_SEED>)\n"
    "          # with a single hard-coded default seed (e.g. 0)\n"
    "      def seed_state(self, seed: int) -> None:\n"
    "          # deterministically build the initial universe from `seed`. Draw ALL\n"
    "          # variation (row counts, chosen values, ordering) from a\n"
    "          # `rng = random.Random(seed)` so the SAME seed always reproduces the\n"
    "          # SAME starting state and a DIFFERENT seed produces a\n"
    "          # different-but-reproducible one.\n"
    "  Wire POST /forge/reset: STATE.seed_state(seed) when a seed is supplied, else\n"
    "  STATE.reset_state(). Endpoints call methods on STATE; they do not run ad-hoc\n"
    ...
    "  - STATE.seed_state(seed) MUST first set `_FORGE_CLOCK = 0` and `_ID_COUNTERS.clear()`\n"
    "    before rebuilding, so every reset (seeded or baseline via reset_state) re-initializes\n"
    "    the universe to a byte-identical state for that seed (same rows, same ids, same\n"
    "    counters) every time. The same seed reproduces identical state; a different seed\n"
    "    yields a different-but-reproducible one.\n"
    "\n"
...
_DOCKERFILE_SYSTEM = (
    ...
    "rewrites the FROM line, the install step, and any port directives anyway,\n"
    "but match this\n"
    ...
    "\n"
    "Output exactly the seven lines above. Never add other install steps: Forge\n"
    "installs only its pinned lock and removes any other pip or apt-get line.\n"
    "Never use a port other than 8000.\n"
    "\n"
